...
<body>
    <script>
        ...
        [HttpPost]
        public async Task<IActionResult> CreateForTicket([FromBody] JsonElement data)
        {
            var userId = User.FindFirstValue(ClaimTypes.NameIdentifier);
            if (string.IsNullOrEmpty(userId))
                return Json(new { success = false, message = "کاربر احراز هویت نشده است." });
            ...
            if (!data.TryGetProperty("projectId", out var projectIdProp) || projectIdProp.ValueKind != JsonValueKind.Number ||
                !data.TryGetProperty("ticketId", out var ticketIdProp) || ticketIdProp.ValueKind != JsonValueKind.Number ||
                !data.TryGetProperty("title", out var titleProp))
            {
                return Json(new { success = false, message = "پروژه، تیکت و عنوان الزامی است." });
            }

            var projectId = projectIdProp.GetInt32();
            var ticketId = ticketIdProp.GetInt32();
            var title = (titleProp.GetString() ?? string.Empty).Trim();
            if (string.IsNullOrWhiteSpace(title))
                return Json(new { success = false, message = "عنوان الزامی است." });
            ...
            var ticket = await _context.ProjectTickets.AsNoTracking()
                .FirstOrDefaultAsync(t => t.Id == ticketId && t.ProjectId == projectId);
            if (ticket == null)
                return Json(new { success = false, message = "تیکت انتخاب‌شده متعلق به این پروژه نیست." });

            var canCreate =
                string.Equals(ticket.CreatedByUserId, userId, StringComparison.Ordinal) ||
            ...
            if (!canCreate)
                return Json(new { success = false, message = "فقط پرسش‌کننده، پاسخ‌دهنده یا سازنده پروژه می‌توانند برای این تیکت تسک تعریف کنند." });
