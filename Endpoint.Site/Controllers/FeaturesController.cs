...
namespace Endpoint.Site.Controllers
{
    ...
    public class FeaturesController : Controller
    {
        ...
        public async Task<IActionResult> EditSpec(int id)
        {
            ...
            EnsureDefaultPageStates(vm);
            return View(vm);
        }

        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> EditSpec(int id, FeatureSpecEditVm vm)
        {
            if (id != vm.Id)
                return NotFound();
            ...
            vm.ApiContracts ??= new List<FeatureApiContractItemVm>();
            vm.BusinessRules ??= new List<FeatureBusinessRuleItemVm>();

            // حذف ردیف‌های خالی کارکرد و قانون
            vm.Functions = vm.Functions.Where(x => !string.IsNullOrWhiteSpace(x.Title)).ToList();
            vm.BusinessRules = vm.BusinessRules.Where(x => !string.IsNullOrWhiteSpace(x.Description)).ToList();
            vm.ApiContracts = vm.ApiContracts.Where(x => !string.IsNullOrWhiteSpace(x.Endpoint)).ToList();
            ...
            // وضعیت‌های صفحه: به‌روزرسانی بر اساس StateType
            foreach (var item in vm.PageStates)
            {
                var existing = feature.PageStates.FirstOrDefault(p => p.StateType == item.StateType);
                if (existing == null)
                {
                    feature.PageStates.Add(new FeaturePageState
                    {
                        ...
                        BehaviorDescription = string.IsNullOrWhiteSpace(item.BehaviorDescription)
                            ? "رفتار این وضعیت هنوز مشخص نشده است."
                            : item.BehaviorDescription.Trim(),
                        HasSeparateDesign = item.HasSeparateDesign
                    });
                }
                else
                {
                    existing.BehaviorDescription = string.IsNullOrWhiteSpace(item.BehaviorDescription)
                        ? "رفتار این وضعیت هنوز مشخص نشده است."
                        : item.BehaviorDescription.Trim();
                    existing.HasSeparateDesign = item.HasSeparateDesign;
                }
            }

            ReplaceCollection(
                feature.ApiContracts,
                vm.ApiContracts,
                (entity, item, order) =>
                {
                    ApplyApiContract(entity, item, order);
                },
                item =>
                {
                    var entity = new FeatureApiContract { FeatureId = feature.Id };
                    ApplyApiContract(entity, item, 0);
                    return entity;
                });
            ...
            TempData["Success"] = "مشخصات فیچر ذخیره شد.";
            return RedirectToAction(nameof(Details), new { id });
        }
        ...
        private static void ReplaceCollection<TEntity, TVm>(
            ICollection<TEntity> existing,
            List<TVm> incoming,
            Action<TEntity, TVm, int> update,
            Func<TVm, TEntity> create)
            where TEntity : class
        {
            existing.Clear();
            for (var i = 0; i < incoming.Count; i++)
            {
                var entity = create(incoming[i]);
                update(entity, incoming[i], i);
                existing.Add(entity);
            }
        }

        private static void EnsureDefaultPageStates(FeatureSpecEditVm vm)
        {
            foreach (FeaturePageStateType stateType in Enum.GetValues(typeof(FeaturePageStateType)))
            {
                if (vm.PageStates.All(p => p.StateType != stateType))
                {
                    vm.PageStates.Add(new FeaturePageStateItemVm
                    {
                        StateType = stateType,
                        BehaviorDescription = "رفتار این وضعیت هنوز مشخص نشده است."
                    });
            ...
            vm.PageStates = vm.PageStates.OrderBy(p => p.StateType).ToList();
        }
